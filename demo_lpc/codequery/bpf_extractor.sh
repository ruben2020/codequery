...
echo "Running cqsearch for $function_name and outputting dependencies to "
status=`cqsearch -s ./myproject.db -t $function_name  -p 7  -l 100 -k 10 -e > $codeq_op`
status=$?
if [ $status -eq 0 ]
then
...
#replace $HOME with base dir
BASE_DIR='/root'
sed -i  -e "s|\$HOME|${BASE_DIR}|g" func.out
