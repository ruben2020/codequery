...
__attribute__ ((__always_inline__)) static inline int send_icmp4_too_big (struct __sk_buff *xdp) {
    int headroom = (int) sizeof (struct iphdr) + (int) sizeof (struct icmphdr);
    if (bpf_skb_adjust_room (xdp, 0 - headroom, BPF_ADJ_ROOM_MAC, BPF_F_ADJ_ROOM_ENCAP_L3_IPV4)) {
        return TC_ACT_SHOT;
    }
...
__attribute__ ((__always_inline__)) static inline int send_icmp6_too_big (struct __sk_buff *xdp) {
...
__attribute__ ((__always_inline__)) static inline int send_icmp_too_big (struct __sk_buff *xdp, bool is_ipv6, int pckt_size) {
